...
export default class Governor {
  ...
  getPlugins() {
    ...
    const isDirectory = folderName =>
      fs
        .statSync(path.resolve(`${this.config.PLUGIN_PATH}/${folderName}`))
        .isDirectory();
  ...
  loadPlugins() {
    ...
    this.getPlugins().forEach(plugin => {
      ...
      if (!fs.existsSync(p)) {
        ...
        logger.error(
          `${plugin.path}/package.json does not have an entry point! 'main'`
        );
      } else {
        ...
        try {
          ...
          logger.error(
            `There has been an error trying to load module ${
              plugin.name
            } : ${err}`
          );
          console.log(err);
